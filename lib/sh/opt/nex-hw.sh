...
(
	tmpa=$(find "$(nx_info_canonize "$1")/" -type f 2>/dev/null) || {
		...
		nx_io_printf -E "'$1' yields no glyphs. The kernel's altar is silent—no files found beneath '$tmpa'. Verify your offering and invoke again."
...
(
	...
	tmpa=$(find "$(nx_info_canonize "/sys/class/backlight/$d")/" -type f 2>/dev/null) || {
		nx_io_printf -E "Backlight device '$d' yields no glyphs. The kernel's altar is silent—no files found beneath /sys/class/backlight. Verify your offering and invoke again."
			...
			-s) {
				test "$2" -ge 0 -a "$2" -lt "$tmpa"  2> /dev/null && {
					...
					if test -d "$freq_path"; then
						...
						nx_io_printf -a "cpu$2 has no cpufreq interface"
					fi
					shift
