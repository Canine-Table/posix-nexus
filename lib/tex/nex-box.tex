...
\nxThemedItems{Envelope}{\faEnvelope}
\nxThemedItems{Phone}{\faPhone}
...
\nxThemedItems{DB}{\faDatabase}
\nxThemedItems{ClipList}{\faClipboardList}
\nxThemedItems{ListUl}{\faListUl}
\nxThemedItems{ListOl}{\faListOl}
\nxThemedItems{List}{\faList}
\nxThemedItems{User}{\faUser}
...
\def\nxBox#1{%
	\edef\nx@tmpa{nx\nxTitle{#1}{ }{ }ArwRay}
	\expandafter\def\csname\nx@tmpa\endcsname##1##2{{\textcolor{nex.#1.back}{\bf\nxEm{##1}{##2}}$\,$\tikz[baseline=-0.5ex]\draw pic [thick] {nxArwRay=#1};~}}
}
...
\def\nxEachArw#1#2#3#4{
	\foreach \x/\y in {#4} {
		\expandafter\csname nx#1Item\expandafter\endcsname\expandafter\csname nx#2ArwRay\expandafter\endcsname#3{\x}\y
	}
}
