...
class PizzaStringBuilder {
    ...
    // Step 3: A getter that reconstructs the string every time
    get value() {
        return this[SECRET_KEY].reduce((a, b) => a + b, "");
    }
}

// Step 4: Wrap the class in a Proxy for no reason at all
const pizzaProxy = new Proxy(new PizzaStringBuilder(), {
    get(target, prop) {
        if (prop === "value") {
            // Step 5: Add a pointless async wrapper
