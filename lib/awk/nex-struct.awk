...
#function nx_escape_index(D1, S, D2,   f, r, rs, logical_idx, physical_idx, offset, b_count) {
#	if (D1 != "") {
#		f = 0
#		S = __nx_else(S, " ")  # Default search pattern
#		D2 = __nx_else(__nx_defined(D2, 1), "\\\\")  # Default escape character
#		#		while (match(D1, S)) {
#			# Update logical and physical indices
#			logical_idx = logical_idx + RSTART
#			physical_idx = logical_idx + offset
#			# Count backslashes preceding the match
#			b_count = nx_count_backslashes(D1, RSTART)
#			# Validate the match
#			if (b_count % 2 == 0) {
#				return logical_idx
#			}
#			# Match is escaped, continue searching
#			offset = nx_calculate_offset(D1, logical_idx)
#			D1 = substr(D1, logical_idx + 1)
#		}
#
#		# No non-escaped match found
#		return -1
#	}
#}

function nx_escape_index(D1, S, N, B1, B2, D2, D3, m, b1, b2, r, rs, l, d, s, rg, i, pi, li, ii)
{
	if (D1 != "") {
		rg = "[\[\]\{\}\(\)\.\+\-\*\?\^\$\|\&]"
		...
		if ((s = length(S)) > 1)
			b1 = 1
		S = __nx_else(nx_escape_str(S, N, B2, D2, D3), "[ \t\n\v\f]")
		i = length(D1)
		D1 = nx_escape_str(D1, N, B2, D2, D3)
		di = length(D1) - i
		li
		pi
		ii
		while (match(D1, S)) {
			r = substr(D1, RSTART + RLENGTH)
			rs = substr(D1, 1, RSTART - 1)
			m = m + RSTART # - nx_count_str(rs, rg)
#			if (match(rs, D3 D3 "+$")) {
#				l = nx_ceiling(RLENGTH / 2)
#				m = m - l
#				if (l % 2 == 1) {
#					b1 = 1
#				} else {
...
#function nx_escape_index(D1, S, N, B1, B2, D2, D3,		m, b1, b2, r, rs, l, sc, so, scl)
#{
#	if (D1 != "") {
#		D3 = __nx_else(__nx_defined(D3, 1), "\\")
#		sc = nx_count_str(S, "[\[\]\{\}\(\)\.\+\-\*\?\^\$\|\&]")
...
#			r = substr(D1, RSTART + RLENGTH)
#			rs = substr(D1, 1, RSTART - 1)
#			scl = nx_count_str(rs, D3 D3 D3 D3) - nx_count_str(rs, "[\[\]\{\}\(\)\.\+\-\*\?\^\$\|\&]")
#			#m = m - scl
#			if ((b1 && match(rs, D3 "+$")) || match(rs, D3 D3 "+$")) {
#				l = nx_ceiling(RLENGTH / 2)
#				m = m - l
#				if (__nx_if(b1, RLENGTH % 4 == 0, RLENGTH % 2 == 1)) {
#					b2 = 1
#				} else {
#					D1 = r
#					continue
