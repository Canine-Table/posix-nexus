...
function nx_tui_return(V1, V2, V3, B, V4)
{
...
function nx_tui_escape(V1, V2, V3, B, V4)
{
	...
	# Ensures proper text flow, advancing to a new line or page when necessary.
		...
		if (V3[V2["cr"]] == "\x0a") {  # Newline: Move to next line.
			...
			V1[V1[0] "_line"] = V1["ln"]++  # Increment line counter.
			V1[V1[0] "_page"] = V1["pg"]  # Maintain current page.
...
function nx_tui_prop(V1, V2, V3, B, V4)
{
...
function nx_tui_machine(V1, V2, V3, B, V4)
{
	...
	for (V2["cr"] = 1; V2["cr"] <= V2["len"]; V2["cr"]++) {
		...
		if (V2["ste"] == "NX_DEFAULT") {
			...
			if (V3[V2["cr"]] ~ /[\x09\x0b\x0d\x08\x0a\x0c\x1b]/) {
				...
				if (++V2["rcnt"] >= V2["col"]) {
					nx_tui_fold(V1, V2, V3, B, V4)  # Move excess content to a new line.
				}
			}
		}
	}
...
function nx_tui(D, V, B,	tok, rec, db)
{
	nx_tui_log_db(db)
	if (D == "") {
		if (B)
	...
	if (nx_is_file(D)) {
		...
		while ((getline tok["len"] < tok["fl"]) > 0) {
			...
			if (nx_tui_machine(V, tok, rec, B, db))
				break
			++tok["ln"]
		}
		close(tok["fl"])
