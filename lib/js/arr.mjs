...
Arr.flatten = (val, sep = '-') => {
	const arr = [];
	function expand(val, str = '') {
		function expandObject(val, str = '') {
			Object.entries(val).forEach(([k, v]) => {
				return expand(v, Str.join(str, k, sep));
			});
		}
		function expandArray(val, str = '') {
			val.forEach(i => {
				return expand(i, str);
			});
		}
		if (Type.isArray(val))
			return expandArray(val, str);
		else if (Type.isObject(val))
			return expandObject(val, str);
		else
			arr.push(Str.join(str, val, sep));
	}
	if (Type.isDefined(val))
		expand(val);
	return arr;
