...
import { nexNode } from './components/node.mjs';
...
nexClass.nodes = (val, obj) => {
	...
	if (! nexNode.isNode(obj.to)) {
		if (val.self instanceof nexNode)
			obj.to = nexNode.body;
		else
			throw new Error(`${obj.inst} instances require a parent to append to`);
	}
	if (! Type.isDefined(parent) && obj.tag !== 'frag')
		obj.tag = val.def;
	else
...
nexClass.idx = obj => {
	if (! (Type.isDefined(obj.id) || Type.isFalse(obj.id)) && nexNode.isElement(obj.to)) {
