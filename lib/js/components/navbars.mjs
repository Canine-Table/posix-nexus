...
export class nexNavbar
{
	...
	constructor(obj) {
		...
		window.addEventListener('hashchange', () => {
			if (Type.isDefined(this.locations[window.location.hash]))
				this.locations[window.location.hash].click()
		});
	}
	...
	addLink(obj) {
		...
		this.locations[`#${Str.lastChar(this.links[this.links.length - 1].href, '#')}`] = this.links[this.links.length - 1];
