...
Dom.isSvg = val => {
	return val instanceof SVGSVGElement || val instanceof SVGGraphicsElement || val instanceof SVGElement || val instanceof SVGAnimatedString;
}

Dom.isElement = val => {
	return Dom.isDocument(val) || Dom.isHtml(val) || Dom.isSvg(val);
}
...
Dom.tagCount = (elm, val) => {
	if (Dom.isElement(elm) && Type.isDefined(val))
		return Dom.byAll(`:scope > ${val}`, elm).length;
}
...
Dom.apply = (obj, elm) => {
	...
	if (Dom.isElement(elm)) {
		...
		if (Type.isDefined(obj.ns)) {
			obj.ns = Dom.ns(obj.ns);
			tag = document.createElementNS(obj.ns, obj.tag);
			if (Type.isDefined(obj.ns) && ! Type.isFalse(obj.xmlns))
				obj.prop.xmlns = obj.ns;
		} else {
...
Dom.parent = (val, cnt = 1) => {
	if (Dom.isElement(val)) {
		if (! Type.isIntegral(cnt) || cnt < 2)
			return val.parentElement;
		return Dom.parent(val.parentElement, --cnt);
	}
}
