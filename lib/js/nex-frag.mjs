...
__nxFrag.route = j => {
	...
	nxData._dfs(j, o => {
		const stem = (o[0][3] > 1)
			? nxDom.GetHook(o[1][o[0][3]][o[0][o[0][2]]], 'self')
			: root;
		const self = o[1][0][o[0][0]];
		if (! nxType.defined(self))
			return;
		if (nxType.isPrimitive(self))
			return stem.appendChild(nxDom.AddHook(stem, 'node.text', [nxDom.Create(`${self}`)])[0]);

		const node = nxDom.Create(nxDom.Frame(self));
		nxDom.SetHook(self, 'self', node);
		...
		//console.log(o[1][o[0][3]][o[0][o[0][2] - 3]])

		//nxDom.AddHook(stem, `node.${self?.tag || nxDom.GetHook(self, 'rule.ghostTag') || 'div'}`, [node])[0]);

		//console.log(node)
		//console.log(self.text)
		if (__ = nxDom.GetHook(self, 'text'))
			node.innerText = __;
