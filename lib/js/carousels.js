...
function addCarousel(obj)
{
	if (isObjectType(obj)) {
		...
		if (! isDefinedType(obj.prop))
			obj.prop = {};
		obj.prop['data-bs-theme'] = isDefinedType(obj.theme, 'dark');
		if (isDefinedType(obj.auto))
			obj.prop['data-bs-ride'] = 'carousel';
		if (isDefinedType(obj.fade))
			obj.clsOut = `${obj.clsOut} carousel-fade`;
		let tag = addTag({
			'tag': 'div',
			'id': `carousel${obj.id}`,
			'to': obj.to,
...
function addCarouselItem(obj)
{
	if (isObjectType(getObjId(`carousel${obj.to}Inner`))) {
		...
		if (! isObjectType(obj.prop))
			obj.prop = {};
		if (isDefinedType(obj.itv) && isUnsignedInteger(obj.itv))
			obj.prop['data-bs-interval'] = obj.itv;
		let tag = addTag({
			'tag': 'div',
