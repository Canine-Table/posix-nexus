...
function addFormTemplateOne(obj)
{
	if (isObjectType(obj)) {
		getPropList(obj, 'form');
		let tag = addTo(getObj(obj.to), addTag({
			'tag': 'form',
			'id': `${obj.id}FormBody`,
			'clsSet': 'container-sm container-md container-lg container-xl container-xxl'
		}));
		addTextForm({
			'lblPre': 'Name',
			'id': `${obj.id}FormName`,
		...
		addFileForm({
			'id': `${obj.id}FormAttachment`,
			'to': `${obj.id}FormBody`,
		});
		addAreaForm({
			'lblFlt': 'Message',
			'id': `${obj.id}FormMessage`,
			'to': `${obj.id}FormBody`,
		});
		addTag({
			'tag': 'button',
			'id': `${obj.id}FormSubmitButton`,
			'to': `${obj.id}FormBody`,
			'clsSet': 'row btn btn-outline-primary gx-1',
			'txtAdd': 'Send',
			'prop': {
				'type': 'submit'
			}
