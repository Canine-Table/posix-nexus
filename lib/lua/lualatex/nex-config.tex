...
\NewDocumentCommand{\nxLnk}{O{} O{Canine-Table} O{Contents} O{}}{
	\global\def\nxS{#3}
	\fancyhead[LO, RE]{%
		\tcbox[on line, size=fbox]{\hyperref[sec:\nxS]{Posix-Nexus}}\hspace{5pt}%
		\tcbox[on line, size=fbox]{\hyperref[sec:#3]{#1}}%
	}
	\hypersetup{
		colorlinks=true,
		linkcolor=nex.primary.back,
		citecolor=nex.primary.link,
		urlcolor=nex.dark.link,
		pdfborder={0 0 0},
...
\NewDocumentCommand{\nxTitle}{O{}O{}O{}O{../lib/svg/}O{Canine-Table}O{Contents}}{%
	...
	\date{\bf\today}
	\maketitle
	\begin{NxMainBox}[light, title=\textbf{Abstract}, sdwA]
