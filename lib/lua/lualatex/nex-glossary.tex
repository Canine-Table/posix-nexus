...
\nxGLSE{fsm}{Finite State Machine (FSM)}{A computational model used to recognize regular languages, consisting of a finite number of states, transitions, and an input alphabet. FSMs are widely used in hardware design, compilers, and network protocols.\cite{fsm-history}}

\nxGLSE{cfl}{Context-Free Language (CFL)}{A type of formal language generated by a context-free grammar (CFG). CFLs are more powerful than regular languages and can be recognized by pushdown automata (PDA), making them useful in programming languages, natural language processing, and syntax analysis.\cite{cfl-history}}

\nxGLSE{dfa}{Deterministic Finite Automaton (DFA)}{A computational model in automata theory used to recognize regular languages. In a DFA, each state has exactly one transition per input symbol, ensuring deterministic behavior.\cite{dfa-history}}

\nxGLSE{nfa}{Non-deterministic Finite Automaton (NFA)}{A computational model in automata theory that recognizes regular languages with non-deterministic behavior. An NFA allows multiple transitions for the same input symbol, including ε (epsilon) transitions.\cite{nfa-history}}
