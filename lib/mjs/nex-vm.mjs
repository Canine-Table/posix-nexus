...
export function nxVm(
	...
	if (block < 2) {
		...
		// Header cannot exceed the block
		const half = block >>> 1;
		if (header >= block)
			header = half;
	}
	...
	const start = Math.ceil(C_STARTING_TOP / size) * size;
	...
	// Stack header view
	const stk = new Uint32Array(buf, 0, C_STARTING_TOP);
	const flt = new Float32Array(buf, S_RESIZE * 4, 1);

	if (doublyFree)
		flags = nxBit32.on(flags, 2);
	if (doublyPool)
		flags = nxBit32.on(flags, 3);
