import { NxStack } from "./batch.d/nex-stack.mjs";
...
export class NxDom
{
	...
	static Route(i32, stack) {
		...
		//const nodes = stack[i32[top]] ??= '';

		if (depth > 2) {
			//this.Create(stack[i32[top - 3]][i32[top - 2]][this.Node], stack[i32[top - 1]][i32[top]]);


			//} else if (depth === 2) {

			//const nodes = stack[i32[top - 1]]
			//const node = nodes[i32[top]] ??= '';
				//const pnodes = stack[i32[top - 2]]
				//const pnode = pnodes[i32[top - 3]] ??= '';
				//console.log(stack[i32[top - 3]], stack[i32[top - 1]])

			//	i32[S_ROOTS],stack, idx, depth, start, stop, stack, i32, stack[start], stack[idx])
			//const node = nodes[idx];
		} else {

			if (nxType.isObject(stack[idx])) {
				console.log(stack[idx])
			if (depth === 1)
				this.Create(stack[start], stack[idx]);
				//console.log(stack[i32[top - 1]][i32[top]] );
				//this.Create(stack[idx][this.Node], stack[i32[top - 1]][i32[top]]);
			}
		}
		/*if (typeof node === 'object') {
			//...
			if (i32[S_INDEX] === 0) {
				console.log(stack, node[top], node)
			}
			console.log(node.twig)
