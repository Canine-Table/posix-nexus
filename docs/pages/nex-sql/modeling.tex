...
\begin{NxLightBox}[title={Crow’s Foot Notation}]
	\begin{tabularx}{\linewidth}{|l|X|X|}
		...
		\hline
		\tikz\draw[->, >=nxMO] (0,0) -- (1,0); & Mandatory-One & Exactly-One \\
		\hline
		\tikz\draw[->, >=nxOM] (0,0) -- (1,0); & Optional-Many & Zero or more \\
		\hline
		\tikz\draw[->, >=nxMM] (0,0) -- (1,0); & Mandatory-Many & One or More \\
		\hline
		\tikz\draw[->, >=nxOO] (0,0) -- (1,0); & Optional-One & Zero or one \\
		\hline
	\end{tabularx}
\end{NxLightBox}
...
\begin{tikzpicture}
	\node (nodeAb) [/nx/shapes/process] {DEPARTMENT}
		node (nodeBb) [/nx/shapes/decision, right=of nodeAb] {1:N}
		node[below of=nodeBb] {\nxLnArcBox[]{(a) Original E-R Model Version}}
		node (nodeCb) [/nx/shapes/process, right=of nodeBb] {EMPLOYEE};
		\draw[<-nxOM] (nodeAb) -- (nodeBb);
		\draw[=>, >=nxMO](nodeBb) -- (nodeCb);
\end{tikzpicture}
\bigskip

\begin{tikzpicture}[>=Stealth]
	\node[/nx/shapes/process] (nodeAa) {DEPARTMENT} node[right=of nodeAa, /nx/shapes/process, xshift=4cm] (nodeAb) {EMPLOYEE};
	...
	\draw[<->, <-nxOO, >=nxMO, draw=none] (nodeAa) -- (nodeAb);

	% Node overlays (not affected by dashed)
	\node[below, yshift=-1cm, /nx/shapes/process, fill=teal!20] (nodeAc) at ($(nodeAa)!0.5!(nodeAb)$) {Minimum cardinality (0-M)};
	\node[/nx/shapes/process, fill=teal!20, above, yshift=1cm] (nodeAd) at ($(nodeAa)!0.5!(nodeAb)$) {Maximum cardinality (N:M)};
...
\nxSections{Example of ID-Dependent Entities}{3}

\begin{tikzpicture}[every node/.style={scale=0.9}]
	\node (nodeAa) {
		\begin{NxLightBox}[width=0.35\linewidth, title={BuildingName}]
			...
			\end{tabularx}
		\end{NxLightBox}
	} node[below=of nodeAa, yshift=-2cm] (nodeAb) {
		\begin{NxLightBox}[width=0.35\linewidth, title={
			\begin{tabularx}{\linewidth}{|X|}
				\hline
				BuildingName \\
				\hline
	...
	\node[right=of nodeAa, xshift=-1cm] (nodeBa) {
		\begin{NxLightBox}[width=0.35\linewidth, title={PaintingName}]
			\begin{tabularx}{\linewidth}{|X|}
				\hline
				Description \\
				\hline
			...
			\end{tabularx}
		\end{NxLightBox}
	} node[below=of nodeBa, yshift=-2cm] (nodeBb) {
		\begin{NxLightBox}[width=0.35\linewidth, title={
			\begin{tabularx}{\linewidth}{|X|}
				\hline
				PaintingName \\
	...
	\node[right=of nodeBa, xshift=-1cm] (nodeCa) {
		\begin{NxLightBox}[width=0.3\linewidth, title={PatientName}]
			\begin{tabularx}{\linewidth}{|X|}
				\hline
				Phone \\
				\hline
			...
			\end{tabularx}
		\end{NxLightBox}
	} node[below=of nodeCa, yshift=-3cm] (nodeCb) {
		\begin{NxLightBox}[width=0.3\linewidth, title={
			\begin{tabularx}{\linewidth}{|X|}
				\hline
				PatientName \\
				\hline
	...
	\node[/nx/lbl/above={nodeAa}] {BUILDING}
		node[/nx/lbl/above={nodeBa}] {PAINTING}
		node[/nx/lbl/above={nodeCa}] {PATIENT};

	\node[/nx/lbl/above={nodeAb}] {APARTMENT}
		node[below=-1mm of nodeAb] {\nxLnArcBox[]{(a) APARTMENT is\\ID-Dependent\\on BUILDING}}
		node[/nx/lbl/above={nodeBb}] {PRINT}
		node[below=-1mm of nodeBb] {\nxLnArcBox[]{(b) PRINT is\\ID-Dependent\\on PAINTING}}
		node[/nx/lbl/above={nodeCb}] {EXAM}
		node[below=-1mm of nodeCb] {\nxLnArcBox[]{(c) EXAM is\\ID-Dependent\\on PATIENT}};

		\begin{scope}[shft/.style={xshift=-2cm}]
			\draw[<->, <-MO, >=OM] ([shft]nodeAa.south east) -- ([shft]nodeAb.north east);
			\draw[<->, <-MO, >=OM] ([shft]nodeBa.south east) -- ([shft]nodeBb.north east);
			\draw[<->, <-nxMO, >=nxMM] ([shft]nodeCa.south east) -- ([shft]nodeCb.north east);
	\end{scope}
\end{tikzpicture}

\nxSections{Non-ID-Dependent Weak Entities}{3}
\begin{center}
	\begin{tikzpicture}[every node/.style={scale=0.9}]
		\node (nodeAa) {
			\begin{NxLightBox}[width=0.35\linewidth, title={
				...
				\end{tabularx}
			\end{NxLightBox}
		} node[below left=-1cm of nodeAa, yshift=-2cm] (nodeAb) {
			\begin{NxLightBox}[width=0.35\linewidth, title={
				\begin{tabularx}{\linewidth}{|X|}
					\hline
					BuildingName \\
				...
				\end{tabularx}
			\end{NxLightBox}
		} node[below right=-1cm of nodeAa, yshift=-2cm] (nodeAc) {
			\begin{NxLightBox}[width=0.35\linewidth, title={
				\begin{tabularx}{\linewidth}{|X|}
					\hline
					BuildingName \\
		...
		\node[/nx/lbl/above={nodeAa}] {AUTO\_MODEL};

		\node[/nx/lbl/above={nodeAb}] {VEHICLE}
			node[below=-1mm of nodeAb] {\nxLnArcBox[]{(a) ID-Dependent Entity}}
			node[/nx/lbl/above={nodeAc}] {VEHICLE}
			node[below=-1mm of nodeAc] {\nxLnArcBox[]{(b) Non-ID-Dependent\\Weak Entity}};

			\begin{scope}[shft/.style={xshift=-2cm}]
				\draw[<-> <-nxOM, >=nxMO] (nodeAa) -- ([shft]nodeAb.north east);
				\draw[<-> <-nxOM, >=nxMO] (nodeAa) -- ([shft]nodeAc.north east);
			\end{scope}
	\end{tikzpicture}
\end{center}
...
\end{NxLightBox}
	...
	\draw[<->, >=nxMO] (memberA) -- (uniformA); %^node[midway, below, yshift=-3.5cm]{(b) Placing a primary Key of the Parent in the Child is a Foreign Key};
\end{tikzpicture}
\end{center}
\end{comment}
