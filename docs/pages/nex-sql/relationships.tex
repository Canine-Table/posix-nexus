...
\begin{NxLightListBox}[title={Primary Key Principles}]
    \nxEachItem{ArrowDark}{
        ...
        {Prefer numbers—numeric keys are processed faster than character types.},
...
\begin{tikzpicture}[line width=1pt]
	...
	\end{scope}
	\node[right=6cm of name.east] (employeeB) {
		\begin{NxLightBox}[%
			width=0.3\linewidth,%
...
\begin{tikzpicture}
	\node (nodeAa) [process] {CLIENT}
		node (nodeBa) [decision, right=of nodeAa] {~}
		node[above=of nodeBa] {\nxLnArcBox{(b) Example Tertiary Relationship}}
		node (nodeCa) [process, right=of nodeBa] {ARCHITECT}
		node (nodeDa) [process, below=of nodeBa] {PROJECT} 
		node[above of=nodeBa] {Assignment};
	\draw (nodeAa) -- (nodeBa) -- (nodeCa);
	\draw (nodeBa) -- (nodeDa);
	\node[right=of nodeCa] (nodeAb) [process] {EMPLOYEE}
		node (nodeBb) [decision, right=of nodeAb] {~}
		node[below of=nodeBb] {Qualification}
		node[above of=nodeBb] {\nxLnArcBox{(a) Example Binary Relationship}}
		node (nodeCb) [process, right of=nodeBb, xshift=2cm] {BADGE};
