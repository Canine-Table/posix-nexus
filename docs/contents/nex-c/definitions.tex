...
\begin{NexMainBox}[light, hdrA, sdwA, crnA, grwB, ssecA]
	...
	\end{NexMainBox}
		...
		\begin{NexListDark}
			\NexItemDark{[GH] should move left 24 bits (\texttt{v << 24}).}
			\NexItemDark{[EF] should move left 8 bits (\texttt{v << 8}).}
			\NexItemDark{[CD] stays where it is.}
			\NexItemDark{[AB] was isolated earlier and now moves to the least significant byte.}
		\end{NexListDark}

		\textbf{Final Assembly Using \texttt{|} (Bitwise OR)}\\
...
\begin{NexMainBox}[light, hdrA, sdwA, crnA, grwB, ssecA]
	\begin{NexMainBox}[dark, crnA, title=\textbf{Summary}]
Now, everything is reversed, so:
	...
	\end{NexMainBox}
		...
		\begin{NexListDark}
			\NexItemDark{\texttt{>>} moves bytes right to extract them.}
			\NexItemDark{\texttt{\&} masks out unwanted bits to keep the correct ones.}
			\NexItemDark{\texttt{<<} moves bytes left to their new positions.}
			\NexItemDark{\texttt{|} combines the results to form the reversed number.}
		\end{NexListDark}
	\end{NexMainBox}
\end{NexMainBox}
